...
struct ConvertFromString<vector<Subtype>> {
    vector<Subtype> operator()(string s)
    {
        ...
        if (Trim(s).empty()) return value_vector;
...
// RunLuaFile & LuaFileReader
//  used by InterpretLine to run a Lua script.
#ifndef CONFIG_NO_LUAFUNC
void RunLuaFile(string filename)
{
    if (luaL_dofile(LuaFunc::Internal::L, filename.c_str()))
        throw runtime_error("Could not load Lua file " + filename + ": " + lua_tostring(LuaFunc::Internal::L, -1));
}
#endif
                     ...
                     string stage)
{
    if (nvm.find(deprecated_name) != nvm.end())
        throw logic_error("Config: the parameter " + deprecated_name + " assigned from " + stage + " is deprecated.");
}
...
void Parameters::InterpretLines(istream& in)
{
    ...
    {
        getline(in, line);
        while (!line.empty() && line.back() == '\\' && in.good())
        {
            getline(in, extra);
...
//  then interprets text within `backticks` as lua expressions as though value is pasted into line
...
void Parameters::InterpretLine(string line)
{
    ...
    // Interpret backticks
    string::size_type lua_start, lua_end;
    while ((lua_start = line.find("`")) != string::npos)
    {
#ifndef CONFIG_NO_LUAFUNC
        lua_end = line.find("`", lua_start + 1);
        if (lua_end == string::npos)
            throw runtime_error("Backticks don't match in line " + line);

        Func<string> expression("", line.substr(lua_start + 1, lua_end - lua_start - 1));
        line.replace(lua_start, lua_end - lua_start + 1, expression());
#else
        throw runtime_error("Config module not built with support for Lua, so cannot use backtick replacement.");
#endif
    }
    ...
    // Helper to include another file.
    auto include = [&](string file)
    {
        ifstream fin(file);
        if (!fin.good())
            throw runtime_error("Could not include file " + file);
        if (_include_depth > 10)
            throw runtime_error("Include depth exceeds 10.");
    ...
    // Helper to include lines beginning with # from the header from another file.
    auto include_header = [&](string file)
    {
        ifstream fin(file);
        if (!fin.good())
            throw runtime_error("Could not include header from file " + file);
        if (_include_depth > 10)
            throw runtime_error("Include depth exceeds 10.");
    ...
    // Helper to include an external Lua script.
    //// TODO given the growing complexity of the config file parser, should config files just be Lua scripts?
    auto include_script = [&](string file)
    {
#ifndef CONFIG_NO_LUAFUNC
        RunLuaFile(file);
#else
        throw runtime_error("Config module not built with support for Lua, so cannot include_script.");
#endif
    };

    // Helper to instantiate a template.
    auto instantiate_template = [&](string m_sweep_name, string m_template_name, string m_template_substitutions, string m_including, string m_including_header)
    {
        // Make sure this is a valid template
        auto templ = std::find(_template_names.begin(), _template_names.end(), m_template_name);
        if (templ == _template_names.end())
        {
            throw runtime_error("Could not find template named " + m_template_name);
        }
        else
        {
            ...
            auto subst = conv(m_template_substitutions);
            if (subst.size() == 0 && _template_params[which].size() == 1)
            {
                subst.push_back(string());
            }
            else if (subst.size() != _template_params[which].size())
            {
                throw runtime_error("Incorrect number of template parameters in line [" + line + "].");
            }
            else
            {
                ...
                {
                    _nvm_sweep_names.back() = m_sweep_name;
                }
                // Otherwise, make a new sweep.
                else
                {
                    _nvm_sweeps.push_back(NameValueMap());
                    _nvm_sweep_names.push_back(m_sweep_name);
                }

                // Include specified files as though at the top of the template definition
                _template_mode = false;
                if (!m_including.empty()) include(m_including);
                if (!m_including_header.empty()) include_header(m_including_header);
                ...
                for (auto& entry : _templates[which])
                {
                    ...
                    if (value.find('<') != string::npos)    // Don't bother trying any argument replacement if there are no <s.
                    {
                        ...
                        value = regex_replace(value, regex("<\\s*\\$Name\\s*>"), m_sweep_name);
                    }
    ...
    else if (regex_match(line, matches, regex("\\[\\s*(\\S+)\\s*\\]\\s*:\\s*(\\w+)\\s*<([\\s\\S]*)>")))
    {
        instantiate_template(matches[1], matches[2], matches[3], "", "");
    }

    else if (regex_match(line, matches, regex("\\[\\s*(\\S+)\\s*\\]\\s*:\\s*(\\w+)\\s*<([\\s\\S]*)>\\s*including\\s*\"([\\s\\S]*)\"")))
    {
        instantiate_template(matches[1], matches[2], matches[3], matches[4], "");
    }

    else if (regex_match(line, matches, regex("\\[\\s*(\\S+)\\s*\\]\\s*:\\s*(\\w+)\\s*<([\\s\\S]*)>\\s*including_header\\s*\"([\\s\\S]*)\"")))
    {
        instantiate_template(matches[1], matches[2], matches[3], "", matches[4]);
    }

    // Assignment
    else if (regex_match(line, matches, regex("(\\w+)\\s*=\\s*([\\s\\S]*)")))
    {
    ...
    // Include statement
    else if (regex_match(line, matches, regex("include\\s*\"([\\s\\S]*)\"")))
    {
        include(matches[1]);
    }

    // Include header statement
    else if (regex_match(line, matches, regex("include_header\\s*\"([\\s\\S]*)\"")))
    {
        include_header(matches[1]);
    }

    // Include script line
    else if (regex_match(line, matches, regex("include_script\\s*\"([\\s\\S]*)\"")))
    {
        include_script(matches[1]);
    }

    // Uninterpretable line
    else
    {
        throw runtime_error("Could not interpret line [" + line + "].");
    }
}
